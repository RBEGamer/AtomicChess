...
bool HardwareInterface::init_hardware(HardwareInterface::HI_HARDWARE_REVISION _hwrev) {
    ...
    if (hwrev == HardwareInterface::HI_HARDWARE_REVISION::HI_HWREV_DK) {
        ...
        if (gcode_interface == nullptr) {
            ...
            gcode_interface = new GCodeSender(
                    ConfigParser::getInstance()->get(ConfigParser::CFG_ENTRY::HARDWARE_MARLIN_BOARD_SERIAL_PORT), baud);


            //REST EEPROM IF SET
            if (ConfigParser::getInstance()->getBool_nocheck(
                    ConfigParser::CFG_ENTRY::HARDWARE_MARLIN_RESET_EEPROM_TO_DEFAULT_DURING_STARTUP)) {
        ...
        if (userboardcontroller_interface == nullptr) {
            ...
            userboardcontroller_interface = new UserBoardController(
                    ConfigParser::getInstance()->get(ConfigParser::CFG_ENTRY::HARDWARE_UBC_SERIAL_PORT), baud);
        }
...
bool HardwareInterface::setTurnStateLight(HardwareInterface::HI_TURN_STATE_LIGHT _state) {
    ...
    ChessPiece::FIGURE HardwareInterface::ScanNFC() {
        ...
        if (hwrev == HardwareInterface::HI_HARDWARE_REVISION::HI_HWREV_DK) {
            if (iocontroller != nullptr) {
                const int retry_count = ConfigParser::getInstance()->getInt_nocheck(
                        ConfigParser::CFG_ENTRY::HARDWARE_UBC_NFC_READ_RETRY_COUNT) + 1;
    ...
    void HardwareInterface::set_speed_preset(HardwareInterface::HI_TRAVEL_SPEED_PRESET _preset) {
        if (hwrev == HardwareInterface::HI_HARDWARE_REVISION::HI_HWREV_DK) {
            ...
            if (gcode_interface != nullptr) {
                ...
                int fr_move = ConfigParser::getInstance()->getInt_nocheck(
                        ConfigParser::CFG_ENTRY::MECHANIC_FEEDRATE_MOVE);
                int fr_travel = ConfigParser::getInstance()->getInt_nocheck(
                        ConfigParser::CFG_ENTRY::MECHANIC_FEEDRATE_TRAVEL);
                //CHECK FOR FEEDRATE
                if (fr_move <= 0 || fr_travel <= 0) {
                    fr_move = 10;
                    fr_travel = 40;
                    LOG_F(ERROR, "cant set speed_preset to fr_move %i or fr_travel %i USING DEFAULT OF 100 FOR BOTH",
                          fr_move, fr_travel);
                }
                //WRITE FEEDRATE TO CONTROLLER MARLIN
                switch (_preset) {
                    case HardwareInterface::HI_TRAVEL_SPEED_PRESET::HI_TSP_MOVE :
                        gcode_interface->set_speed_preset(fr_move);
                        break; //MM per MINUTE
                    case HardwareInterface::HI_TRAVEL_SPEED_PRESET::HI_TSP_TRAVEL :
                        gcode_interface->set_speed_preset(fr_travel);
                        break;
                    default:
                        break;
                }
    ...
    void HardwareInterface::set_fan_state(HardwareInterface::HI_FAN _fan, bool _state) {
        if (hwrev == HardwareInterface::HI_HARDWARE_REVISION::HI_HWREV_DK) {
            ...
            if (_fan == HI_FAN::HI_FAN_MOTOR_DRIVER) {
                fan_index = ConfigParser::getInstance()->getInt_nocheck(
                        ConfigParser::CFG_ENTRY::HARDWARE_MARLIN_COOLING_FAN_DRIVER_INDEX);
            }
            //SKIP IF DISBALED
            if (fan_index < 0) {
                return;
