...
void UserBoardController::set_led(const int _color) {
    if (_color == current_set_led_value) {
        return;
    }
    current_set_led_value = _color;
    for (int i = 0; i < GENERAL_UBC_COMMAND_RESPONSE_RETRY; i++) {
        ...
        // SPLIT REPSONSE
        if(readres.rfind("ok") != std::string::npos){
            break;
        }else{
            LOG_F(WARNING, "UBC_COMMAND_LED ERRORCODE %s", readres.c_str());
        }
    }
}

void UserBoardController::set_servo(const int _pos) {
    if(_pos == current_set_servo_value){
        return;
    }
    current_set_servo_value = _pos;
    ...
    for (int i = 0; i < GENERAL_UBC_COMMAND_RESPONSE_RETRY; i++) {
        ...
        if(readres.rfind("ok") != std::string::npos){
            ...
            LOG_F(WARNING, "UBC_COMMAND_SERVO ERRORCODE %s", readres.c_str());
        }
    }
 //   std::this_thread::sleep_for(std::chrono::milliseconds(1000));
}
...
ChessPiece::FIGURE UserBoardController::read_chess_piece_nfc() {
    ...
    const int retry_count =
            ConfigParser::getInstance()->getInt_nocheck(ConfigParser::CFG_ENTRY::HARDWARE_UBC_NFC_READ_RETRY_COUNT) + 1;
...
void UserBoardController::set_led_hsv(int _color) {
    ...
    for (int i = 0; i < GENERAL_UBC_COMMAND_RESPONSE_RETRY; i++) {
        ...
        if(readres.rfind("ok") != std::string::npos){
            ...
            LOG_F(WARNING, "UBC_COMMAND__HSV ERRORCODE %s", readres.c_str());
        }
    }
}
