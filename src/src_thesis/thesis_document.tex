...
\tightlist
\item
  docker bzw docker compose Alle aufgelisteten Services werden in
  seperaten Containern betrieben. Die Containervirtualisierung geschieht
  mittels der Software \passthrough{\lstinline!Docker!}. Diese stellt
  ein einfaches Interface zur Erstellung von Containern und der
  Verwaltung dieser. Um einen Container auf dem System starten zu
  können, muss dieser zunächst aus einem Image heraus erstellt werden.
  Diese Image wird mittels einer \passthrough{\lstinline!Dockerfile!}
  beschrieben und besteht aus einer Reihe an Kommandos, welche den
  Aufbau des Images beschreiben.
\end{itemize}
