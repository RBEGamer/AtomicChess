...
Das dafür gewählte Projekt beinhaltet die Konstruktion und
...
Dies soll mittels eines kompakten und minimalistischem Designs
...
Individualisierung benötigen. Ein Nachteil dessen ist, dass dadurch auch
...
abnehmbare Antenne besitzt. Dieses Modul wurde vom Autor der hier
vorliegenden Arbeit bereits in anderen Projekten eingesetzt und erwies
sich dort als zuverlässig.
...
eine Achse zu bewegen, nicht mehr als 45 Ncm betragen muss.
...
Diese Funktionalität wird von dem für dieses Projekt implementierten
...
Allgemein kann eine Datentransfer über den \gls{spi} drei Mal
...
Als Außenmaße wurden 620mm x 620mm x 170mm (Länge, Breite, Höhe)
gewählt. Das Außenmaß ergab sich aus der Berechnung der benötigten
Spielfeldgröße, der Parkpositionen und der gegebenen Stangenbreite. Die
Schachfiguren besitzen einen maximalen Durchmesser von 22mm. Damit
Figuren aneinander ohne Berührung vorbeigeführt werden können, ist somit
eine Größe von mindestens 44mm für ein Feld nötig. Da eine Distanz
...
möglich sind, wurde hierfür eine zusätzliche Toleranz von 13mm ergänzt
und somit ein Idealmaß von 57mm Seitenlänge pro Feld errechnet. Bei
einem vollständigen Schachtisch ergibt sich daraus eine Schachfeldgröße
von 456mm x 456mm. Für die Parkpositionen wurden zusätzlich noch einmal
30mm berechnet mit einem Abstand zum Feld von weiteren 37mm. Somit
ergibt sich, wenn man das Feld quadratisch auslegt, eine Seitenlänge von
590mm. Als Plattenmaß wurde 620mm gewählt, um eine Toleranz für die
...
aufgewertet wird. Zudem wurden im Inneren noch zusätzlich
\gls{led}-Streifen verlegt, welche dank des verwendeten TMC-Boards
einfach angeschlossen werden konnten. Die weißen Glaselemente streuen
das Licht günstiger und ermöglichen so ein unterschwelliges Leuchten.
...
Eine Höhe von 3mm darf aufgrund des Magnetismus zwischen Schlitten und
...
Die Konstruktion ist zudem stabiler und erleichtert das Einschließen und
...
\passthrough{\lstinline!\_readnfc\_res\_\_empty\_ok\_!}. Auch hier wird
...
Hierzu wurde ein zusätzlicher
\passthrough{\lstinline!Mongo!}\cite{mogodb} Datenbank Service
erstellt, in welchem diese Daten abgelegt werden. Zusätzlich zu den
Spieldaten (Spiele, Spielstände, Statistiken) werden auch die
Nutzerprofile speichert. Ein Profil wird beim ersten Anmeldevorgang
erstellt und enthält neben den Profilinformationen te-\gls{id}, Namen,
Spielertyp) auch die Referenzen auf die gewonnenen und verlorenen
...
eigentlichen Software benötigt wird. Auch existieren bereits
...
gestartet werden. Solche Funktionalitäten deckt das leichtgewichtige
Tool \passthrough{\lstinline!docker-compose!}\cite{dockercompose}
ab. Durch eine entsprechende Konfigurationsdatei kann ein so genannter
Stack aus mehreren Containern aufgebaut werden.
...
Somit muss mittels eines Algorithmus \ref{ATC_ChessMoveAlgorithm}
...
Trotz der Einfachheit der Bedienung und dem meist nur also
...
Die anschließende Implementierung der Backend-Logik des Unter-Interfaces
bestand in der Verbindung der in \gls{qml} erstellten Bedienelemente
durch den \passthrough{\lstinline!Qt Design Studio!}-Editor mit der
User-Interface Backend Logik. Diese beschränkt sich auf die
Initialisierung des Fensters und das anschließende Laden und Darstellen
des \gls{qml} Codes. Die Backend-Logik Funktionalitäten sind in einem
...
Nicht nur der Aufbau der Komponenten erfolgte modular, sondern auch das
