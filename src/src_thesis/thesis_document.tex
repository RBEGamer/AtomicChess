...
\hypertarget{machbarkeitsanalyse-und-verifikation-der-ausgewuxe4hlter-technologien}{%
\chapter{Machbarkeitsanalyse und Verifikation der ausgewählter
Technologien}\label{machbarkeitsanalyse-und-verifikation-der-ausgewuxe4hlter-technologien}}
...
\hypertarget{buildroot-framework}{%
\section{Buildroot-Framework}\label{buildroot-framework}}
...
sondern auf einer soliden Basis aufbauen. Allgemien soll hier auf ein
...
erzeugen. Auf seiten der Entwicklung ist eine Toolchain notwendig, mit
...
sollte der C++ Compiler mindesten den C++17 Standart untersützen.
...
stark, vor allem wärend der ersten Verwendung.
...
größere Einsteigshürde, durch ein komplexes Layer-System dar. Es bietet
...
\passthrough{\lstinline!Yocot!}-Projekt unterstüzt hierbei das erstellen
einzelner Pakete, somit müssen nur Änderungen neu gebaut werden. Da hier
nur eine minimale Anzahl von Paketen benötigt werden, somit hält sich
dieser Bauvorgang zeitlich in grenzen und ist allgemein für diese
...
dem Quellcode, zwei weitere Dateien notwenig. Die
...
\hypertarget{nfc-technologie}{%
\section{NFC Technologie}\label{nfc-technologie}}
...
Figur-Typ (Bauer, König, Dame, ..) und die Figur-Farbe (schwarz, weiss)
vom System erkannt werden. Da hier keine aufwendige Elektronik
entwickelt werden soll, sondern auf Standard-Komponenten zurückgegriffen
werden soll, scheidet ein komplexes \gls{hf} Antennen-Array unter dem
Schachfeld aus, wie es bei einigen kommerziellen Produkten umgesetzt
ist. Eine einfache 8x8 Matrix aus Drucktastern oder Hall-Effekt-Sensore
scheidet ebenfalls aus, da hier die Eingabe über den Benutzer erfolgt
und nur Rückschlüsse auf die veränderte Figur anhand einer Manuellen
Bewegung der Figur nachvollzogen werden kann.

Somit eignet sich hier die \gls{nfc} Technologie, welche auch bei
modernen Smartphone eingesetzt wird. Hierzu werden kleine \gls{nfc}-Tags
...
welchen maximaler Abstand nötig ist um solch ein Tag scannen zu können.
Auch ist der Abstand zwischen den einzelnen Tags entscheident, wie nah
...
und sind somit ein Standart-Produkt. Im Test stellte sich heruas, dass
...
einen Durchmesser von 22mm haben und somit ein einzelnes Schachfeld ein
...
vernachlässigbar klein ist. Es ist auch davon auszugehen, das die Kraft
welche von dem Motoren benötigt wird um eine Achse zu bewegen nicht mehr
als 45Ncm beträgt.

Dies entspricht den Werten einer X-Y-Achsenkonfiguration, wie sie in
einem handelüberlichen 3D-Drucker zu finden ist und welche mit
...
dass der Motor einen Schritt ausführt. Je nach gewälten Motor entspricht
dies einer Rotation um 1.8 Grad und dies reicht somit für die
...
für diesen Anwendungsfall als nicht Ideal heraus. Um diese Problem zu
...
\hypertarget{anmerkungen-compiler}{%
\subsection{Anmerkungen Compiler}\label{anmerkungen-compiler}}

Die Controller-Software wurde in C++ erstellt und verwendet Features des
C++ 17 Standart:
...
Diese Features werden im \gls{ipc} Modul (s.u.) , sowie einigen
verwendeten Bibliotheken verwendet. Auf dem Host-Entwicklungssystem,
sowie dem eingebetteten System wurde der GCC-Compiler mit der Version
\passthrough{\lstinline!10.2!} installiert und wird für das Erstellen
